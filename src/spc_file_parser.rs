...
fn parse_spc_header(data: &[u8]) -> Option<SPCFileHeader> {
    ...
    Some(SPCFileHeader {
        ...
        spc_register: SPCRegister {
            pc: ((data[0x25] as u16) << 8) | data[0x26] as u16, // TODO: エンディアンは？
            a: data[0x27],
            x: data[0x28],
            y: data[0x29],
            psw: data[0x2A],
        ...
        generate_year: ((data[0xA0] as u16) << 8) | (data[0xA1] as u16), // TODO: エンディアンは？
        duration: ((data[0xA9] as u16) << 8) | (data[0xAA] as u16),      // TODO: エンディアンは？
