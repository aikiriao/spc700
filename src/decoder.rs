...
impl Decoder {
    ...
    /// デコードアドレスの更新
    pub fn set_address(&mut self, ram: &[u8], dir_address: usize) {
        self.decode_start_address = make_u16_from_u8(&ram[dir_address..(dir_address + 2)]) as usize;
        self.decode_loop_address =
            make_u16_from_u8(&ram[(dir_address + 2)..(dir_address + 4)]) as usize;
    }

    /// キーオン時の処理
    pub fn keyon(&mut self, ram: &[u8], dir_address: usize) {
        self.end = false;
        self.set_address(ram, dir_address);
        self.decode_read_pos = self.decode_start_address;
    ...
    /// 1サンプル出力
    pub fn process(&mut self, ram: &[u8], pitch: u16) -> i16 {
        let next_block;
